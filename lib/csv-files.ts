...
import { fileTransferConfig } from "@/constants/app-config";

function filename() {
  return `${fileTransferConfig.csvFilePrefix}-${new Date().toISOString().replace(/[:.]/g, "-")}.csv`;
}
...
export async function chooseCsvFile() {
  ...
  if (asset.size && asset.size > fileTransferConfig.maxImportBytes) throw new Error("Choose a CSV file smaller than 5 MB.");
