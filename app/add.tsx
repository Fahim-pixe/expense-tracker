...
import { isValidDate, parseAmountToCents, type TransactionType } from "@/lib/finance";
...
export default function AddTransactionScreen() {
  ...
  const save = () => {
    if (amountCents <= 0) {
      Alert.alert("Add an amount", "Enter an amount greater than zero before saving.");
      return;
    }
    if (!categoryId) {
    ...
    addTransaction({ type, amountCents, categoryId, note: note.trim(), date });
    router.back();
  };
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <FlatList
          ...
          ListHeaderComponent={
            <View>
              <View style={styles.header}>
                <Pressable onPress={() => router.back()} accessibilityRole="button" accessibilityLabel="Close" style={({ pressed }) => [styles.closeButton, { backgroundColor: colors.surface, borderColor: colors.border }, pressed && { opacity: financeUi.opacity.pressed }]}>
              ...
              <View style={styles.amountBlock}>
                ...
                <View style={styles.amountInputRow}>
                  ...
                  <TextInput autoFocus value={amount} onChangeText={setAmount} placeholder="0.00" placeholderTextColor={colors.muted} keyboardType="decimal-pad" style={[styles.amountInput, { color: colors.foreground }]} returnKeyType="done" />
                </View>
              </View>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable onPress={() => setCategoryId(item.id)} style={({ pressed }) => [styles.categoryChoice, { backgroundColor: selected ? `${colors.primary}10` : colors.surface, borderColor: selected ? colors.primary : colors.border }, pressed && { opacity: financeUi.opacity.pressed }]}>
          ...
          ListFooterComponent={
            <View style={styles.footerFields}>
              ...
              <View style={[styles.inputCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                ...
                <TextInput value={note} onChangeText={setNote} placeholder="Add a note (optional)" placeholderTextColor={colors.muted} style={[styles.detailInput, { color: colors.foreground }]} returnKeyType="done" />
              </View>
              <View style={[styles.inputCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <MaterialIcons name="calendar-today" size={19} color={colors.muted} />
                <TextInput value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" placeholderTextColor={colors.muted} autoCapitalize="none" style={[styles.detailInput, { color: colors.foreground }]} returnKeyType="done" />
              </View>
              <Pressable onPress={save} style={({ pressed }) => [styles.saveButton, { backgroundColor: colors.primary }, pressed && styles.savePressed]}>
                <Text style={[styles.saveButtonText, { color: colors.background }]}>Save {type === "income" ? "income" : "expense"}</Text>
              </Pressable>
            </View>
          }
        />
