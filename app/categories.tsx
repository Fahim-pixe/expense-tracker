...
export default function CategoriesScreen() {
  ...
  const remove = (category: FinanceCategory) => {
    ...
    Alert.alert(`Remove ${category.name}?`, "Transactions using this category must be deleted first.", [
      ...
      { text: "Remove", style: "destructive", onPress: () => { if (!deleteCategory(category.id)) Alert.alert("Category in use", "Delete transactions that use this category before removing it."); } },
    ]);
  };
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <View style={styles.header}><Pressable onPress={() => router.back()} accessibilityLabel="Close" style={({ pressed }) => [styles.closeButton, { backgroundColor: colors.surface, borderColor: colors.border }, pressed && { opacity: financeUi.opacity.pressed }]}><MaterialIcons name="close" size={22} color={colors.foreground} /></Pressable><Text style={[styles.headerTitle, { color: colors.foreground }]}>Categories</Text><View style={styles.closeButton} /></View>
            ...
            <View style={[styles.addRow, { backgroundColor: colors.surface, borderColor: colors.border }]}><TextInput value={name} onChangeText={setName} placeholder="Category name" placeholderTextColor={colors.muted} style={[styles.categoryInput, { color: colors.foreground }]} returnKeyType="done" onSubmitEditing={add} /><Pressable onPress={add} disabled={!name.trim()} accessibilityLabel="Add category" style={({ pressed }) => [styles.addButton, { backgroundColor: name.trim() ? colors.primary : colors.border }, pressed && { opacity: financeUi.opacity.pressed }]}><MaterialIcons name="add" size={22} color={colors.background} /></Pressable></View>
            <Text style={[styles.fieldLabel, { color: colors.muted }]}>YOUR {type.toUpperCase()} CATEGORIES</Text>
          </View>
        }
        renderItem={({ item }) => <View style={[styles.categoryRow, { backgroundColor: colors.surface, borderColor: colors.border }]}><CategoryIcon category={item} /><Text style={[styles.categoryName, { color: colors.foreground }]}>{item.name}</Text>{item.isDefault ? <Text style={[styles.defaultLabel, { color: colors.muted }]}>Default</Text> : <Pressable onPress={() => remove(item)} accessibilityLabel={`Remove ${item.name}`} style={({ pressed }) => [styles.removeButton, pressed && { opacity: financeUi.opacity.pressed }]}><MaterialIcons name="delete-outline" size={21} color={colors.error} /></Pressable>}</View>}
      />
    </ScreenContainer>
  );
}
